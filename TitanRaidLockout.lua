...
local LOCALIZED_CATA_HEROIC_NAMES = {
    ...
    ["TVP"] = GetRealZoneText(657),
    ["LCOT"] = GetRealZoneText(755),
    ["TSC"] = GetRealZoneText(725),
    ["ZGB"] = GetRealZoneText(859),
    ["GBL"] = GetRealZoneText(670),
    ["HOOR"] = GetRealZoneText(644),
    ["ZAN"] = GetRealZoneText(568),
    ["WOE"] = GetRealZoneText(939),
    ["HOTW"] = GetRealZoneText(940),
    ["ENDT"] = GetRealZoneText(938)
}
...
local LOCALIZED_CATA_RAID_NAMES = {
    ...
    ["BTWT"] = GetRealZoneText(671),
    ["BHLD"] = GetRealZoneText(757),
    ["TFW"] = GetRealZoneText(754),
    ["FLDS"] = GetRealZoneText(720)
}
...
function TRaidLockout_SetButtonText()
    ...
    local raidsTableCata10 = {}
    local raidsTableCata25 = {}
    ...
    for k, v in pairs(raidsTableCata) do
        raidsTableCata10[k] = {v[1], false}
    end

    for k, v in pairs(raidsTableCata) do
        raidsTableCata25[k] = {v[1], false}
    end
    ...
    if showUnlocked then -- Show green abbr
            ...
            if show10manRaids then
                ...
                for savedIndex = 1, numSaved do
                    ...
                    if maxPlayers == 10 and TitanUtils_TableContainsValue(LOCALIZED_ALL_RAID_NAMES, name) then
                        ...
                        for key, subTable in pairs(raidsTableCata10) do
                            if name == LOCALIZED_ALL_RAID_NAMES[key] then
                                buttonText = buttonText .. " " .. subTable[1]
                                if showRaidSize then buttonText = buttonText .. "10" end
                                subTable[2] = true
            ...
            if showColoredText then
             ...
             for savedIndex = 1, numSaved do
                ...
                if maxPlayers == 25 and TitanUtils_TableContainsValue(LOCALIZED_ALL_RAID_NAMES, name) then
                    ...
                    for key, subTable in pairs(raidsTableCata25) do
                        if name == LOCALIZED_ALL_RAID_NAMES[key] then
                            buttonText = buttonText .. " " .. subTable[1]
                            if showRaidSize then buttonText = buttonText .. "25" end
                            subTable[2] = true
        ...
        if show10manRaids then
            for index, subTable in pairs(raidsTableCata10) do
                if not subTable[2] then
                    buttonText = buttonText .. " " .. subTable[1]
                    if showRaidSize then buttonText = buttonText .. "10" end
                end
        ...
        for index, subTable in pairs(raidsTableCata25) do
            if not subTable[2] then
                buttonText = buttonText .. " " .. subTable[1]
                if showRaidSize then buttonText = buttonText .. "25" end
            end
...
function TRaidLockout_ToolTip_StringFormat_PlayerCharLockouts(isPlayerChar, showAllChars, numSaved, charData)
    ...
    if isPlayerChar and numSaved < 1 or showAllChars and numSaved < 1 then
        ...
        -- Then show any cata raids
        local cataResultsText = TRaidLockout_ToolTip_InstanceDataLoop(LOCALIZED_CATA_RAID_NAMES, charData,
            COLOR.orange, COLOR.darkorange)

        resultText = resultText .. heroicsResultsText .. classicResultsText .. tbcResultsText .. wotlkResultsText .. cataResultsText
    end
